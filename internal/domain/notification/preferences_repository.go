...
func (r *pgRepository) GetPreferences(ctx context.Context, userID string) (Preferences, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT user_id, email_digest, push_enabled, sms_enabled,
		       morning_digest_enabled, evening_digest_enabled, streaks_enabled,
		...
		&p.MorningDigestEnabled, &p.EveningDigestEnabled, &p.StreaksEnabled,
...
func (r *pgRepository) UpsertPreferences(ctx context.Context, userID string, u UpdatePreferences) (Preferences, error) {
	...
	err := r.db.QueryRow(ctx, `
		INSERT INTO notification_preferences (
			...
			COALESCE(@morningDigestEnabled::boolean, @defMorningDigestEnabled),
			COALESCE(@eveningDigestEnabled::boolean, @defEveningDigestEnabled),
		...
		ON CONFLICT (user_id) DO UPDATE SET
			...
			morning_digest_enabled = COALESCE(@morningDigestEnabled::boolean, notification_preferences.morning_digest_enabled),
			evening_digest_enabled = COALESCE(@eveningDigestEnabled::boolean, notification_preferences.evening_digest_enabled),
		...
		pgx.NamedArgs{
			...
			"morningDigestEnabled":    u.MorningDigestEnabled,
			"eveningDigestEnabled":    u.EveningDigestEnabled,
			...
			"defMorningDigestEnabled": DefaultMorningDigestEnabled,
			"defEveningDigestEnabled": DefaultEveningDigestEnabled,
